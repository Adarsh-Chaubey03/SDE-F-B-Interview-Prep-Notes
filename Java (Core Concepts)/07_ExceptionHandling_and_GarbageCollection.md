...
# Quick Revision — Q89–100

| Q | Key Point |
|---|---|
| 89 | Exception = abnormal condition that disrupts normal program flow |
| 90 | Checked → compiler requires handling/declaring; unchecked → compiler does not |
| 91 | Exception generally represents application conditions; Error represents serious JVM/runtime failures |
| 92 | `try` = risky code, `catch` = handling, `finally` = cleanup |
| 93 | Multiple `catch` blocks are allowed; specific catches must come before general ones |
| 94 | `finally` can exist without `catch`; it normally executes but is not an absolute guarantee |
| 95 | `throw` actually throws an exception; `throws` declares possible exceptions |
| 96 | Custom exception = class extending `Exception` or `RuntimeException` |
| 97 | GC automatically reclaims memory from unreachable objects |
| 98 | GC identifies reachable objects from GC roots and reclaims unreachable objects |
| 99 | Stack = thread-specific frames; Heap = objects/arrays; Method Area = class/interface metadata |
| 100 | Minor GC traditionally targets young generation; major targets old generation; full GC is broader |
...
2. Errors can technically be caught, but serious Errors are generally not recovery targets.
3. `finally` normally executes, but abnormal JVM termination can prevent it.
4. Specific `catch` blocks must come before general ones.
5. `throw` actually throws; `throws` declares.

### JVM & Garbage Collection

6. **Eligible for GC ≠ immediately collected.**
7. `System.gc()` only requests/suggests GC; it does not guarantee immediate collection.
8. Stack is per-thread; heap is shared and primarily GC-managed.
9. Method Area is a JVM specification concept; HotSpot uses Metaspace for its implementation.
10. Major GC and full GC are not universally synonymous.
...
- **Q90** — Checked vs unchecked
- **Q91** — Error vs Exception
- **Q92** — `try`/`catch`/`finally`
- **Q95** — `throw` vs `throws`
- **Q96** — Custom exception

### JVM & Garbage Collection

- **Q97** — Garbage collection
- **Q98** — How GC works
- **Q99** — Stack vs Heap vs Method Area
- **Q100** — Minor/Major/Full GC

## One Connected GC Concept
...
Memory reclaimed
     ↓
Collector-specific cleanup/compaction
```
